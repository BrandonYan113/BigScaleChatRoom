...
def ious_(altrbs: np.array, bltrbs: np.array, negative=False):
    '''
    Args:
        altrbs: list or np.array[m, 4], represent bbox left_top and right_bottom, [x0, y0, x1, y1]
        bltrbs: list or np.array[n, 4], represent bbox left_top and right_bottom, [x0, y0, x1, y1]
    Returns: m * n np.array
    '''
    area1 = (altrbs[:, 2] - altrbs[:, 0]) * (altrbs[:, 3] - altrbs[:, 1])
    area2 = (bltrbs[:, 2] - bltrbs[:, 0]) * (bltrbs[:, 3] - bltrbs[:, 1])

    altrbs = np.expand_dims(altrbs, 1)   # n, 1, 4
    area1 = np.expand_dims(area1, 1)
    l1, r1 = altrbs[:, ..., 0], altrbs[:, ..., 2]   # n, 1
    t1, b1 = altrbs[:, ..., 1], altrbs[:, ..., 3]   # n, 1
    l2, r2 = bltrbs[:, 0], bltrbs[:, 2]     # m
    t2, b2 = bltrbs[:, 1], bltrbs[:, 3]     # m
    XA = np.where(l1 > l2, l1, l2)  # n, m
    YA = np.where(t1 > t2, t1, t2)
    XB = np.where(r1 > r2, r2, r1)
    YB = np.where(b1 > b2, b2, b1)

    XBA = XB - XA
    YBA = YB - YA
    if negative:
        flag = (XBA <= 0) | (YBA <= 0)
        intersection = XBA * YBA
        union = area1 + area2 - intersection
        iou = intersection / union
        flag |= union <= 0
        iou[flag] = -10
    else:
        intersection = XBA.clip(min=0) * YBA.clip(min=0)
        union = area1 + area2 - intersection
        flag = union <= 0
        union[flag] = 1.
        iou = intersection / union
